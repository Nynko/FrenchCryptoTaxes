...
pub fn calculate_weigted_price(sell_price: Decimal, current_cost_basis: Decimal, pf_total_value: Decimal) -> Decimal{
...
fn calculate_new_portfolio(
    ...
    let fee = if tx.fee.is_some() && tx.fee_price.is_some() {tx.fee.unwrap() * tx.fee_price.unwrap()} else {dec!(0.00)};
...
mod tests {
    ...
    fn simple_transfer_with_fee() {
        ...
        let mut tx = Transaction::Transfer { tx: TransactionBase{
            ...
            timestamp: Utc::now(),
            is_taxable:false
        }, from: &from, to, amount: 1, price_eur: price_eur_btc, pf: init_pf };
    ...
    fn simple_trades() {
        ...
        let mut tx = Transaction::Trade { tx: TransactionBase{
            id: String::from("test"),
            fee: None,
            fee_price:None,
            timestamp: Utc::now(),
            is_taxable:true
        }, from: &btc_wallet, to: &eur_wallet, sold_amount: 5, bought_amount: 20000, bought_price_eur: dec!(1), pf: init_pf };
    ...
    fn simple_two_trades() {
        ...
        let mut tx = Transaction::Trade { tx: TransactionBase{
            ...
            timestamp: Utc::now(),
            is_taxable:true
        }, from: &btc_wallet, to: &eur_wallet, sold_amount: 1, bought_amount: 450, bought_price_eur: dec!(1), pf: init_pf };
        ...
        let mut tx2 = Transaction::Trade { tx: TransactionBase{
            id: String::from("test2"),
            fee: None,
            fee_price:None,
            timestamp: Utc::now(),
            is_taxable:true
        }, from: &btc_wallet, to: &eur_wallet, sold_amount: 1, bought_amount: 1300, bought_price_eur: dec!(1), pf: init_pf2 };
