...
async fn fetch_data<T:for<'a> Deserialize<'a>>(url_path: &String, params: &mut HashMap<&str,String>, api_key: &String, api_secret: &String) -> Result<Response<T>,Error>{
    ...
    let encoded = form_urlencoded::Serializer::new(String::new()).extend_pairs(params.clone()).finish();
...
async fn fetch_ledger_data(api_key: &String,api_secret:&String, counter: &mut u8, tier: &Tier ) -> Result<Vec<LedgerEntry>, ApiError>{
    ...
    let ledger_response: Response<LedgersInfo> = fetch_data(&url_path,&mut params,&api_key,&api_secret).await.map_err(|e| ApiError::new(e.to_string()))?;
    *counter+=2;
    ...
    for offset in (50..count).step_by(50){
        ...
        let response: Response<LedgersInfo> = fetch_data(&url_path,&mut params,&api_key,&api_secret).await.map_err(|e| ApiError::new(e.to_string()))?;
        *counter+=2;
...
async fn fetch_deposit_data(api_key: &String,api_secret:&String, counter: &mut u8, tier: &Tier ) -> Result<HashMap<String,Deposit>, ApiError>{
    ...
    while next_cursor.is_some(){
        ...
        let deposit_response: Response<DepositInfo> = fetch_data(&url_path,&mut params,&api_key,&api_secret).await.map_err(|e| ApiError::new(e.to_string()))?;
        *counter+=1;
...
async fn fetch_withdraw_data(api_key: &String,api_secret:&String, counter: &mut u8, tier: &Tier ) -> Result<HashMap<String,Withdrawal>, ApiError>{
    ...
    while next_cursor.is_some(){
        ...
        let deposit_response: Response<WithdrawalInfo> = fetch_data(&url_path,&mut params,&api_key,&api_secret).await.map_err(|e| ApiError::new(e.to_string()))?;
        *counter+=1;
...
#[tokio::main]
pub async fn fetch_history_kraken(tier: Tier) -> Result<(Vec<LedgerEntry>, HashMap<String, Deposit>,HashMap<String,Withdrawal>), ApiError> {
    ...
    // let sleep_time
    let trade_history: Vec<LedgerEntry> = fetch_ledger_data(&api_key,&api_secret, &mut api_counter, &tier).await?;
    let deposits : HashMap<String, Deposit> = fetch_deposit_data(&api_key,&api_secret, &mut api_counter, &tier).await?;
    let withdrawals: HashMap<String,Withdrawal> = fetch_withdraw_data(&api_key,&api_secret, &mut api_counter, &tier).await?;
...
impl Tier {
    ...
    pub fn get_sleep_time(&self, step: u8)-> Duration{
        match self {
            ...
            Tier::Intermediate => Duration::from_secs_f32(step as f32 /Self::INTERMEDIATE_DECAY_RATE),
